...
echo "#-- Retrieve HOST BUFFER ADAPTERS ------------------------"
PCI_ADDRS=$(lspci | grep -i "Fibre Channel" | awk '{print $1}')
readarray -t PCI_ADDRS <<< "${PCI_ADDRS}"
if [[ ${#PCI_ADDRS[@]} = 0 ]]; then
    echo "[WARNING-ERROR] No Fibre Channel host buffer adapter found!"
    exit 1
elif [ ${VERBOSE} -gt 0 ]; then
    echo "[INFO] Found ${#PCI_ADDRS[@]} Fibre Channel ports"
fi
echo "[INFO] Selecting Fibre Channel HBA located at PCI address '${PCI_ADDRS[0]}'"

echo "#-- Retrieve SCSI_HOST -----------------------------------"
SCSI_HOST_NAME=$(basename /sys/bus/pci/devices/0000:${PCI_ADDRS[0]}/host*)
if [ $? -ne 0 ]; then
    echo "[ERROR] Failed to retrieve the SCSI host name for PCI address '${PCI_ADDR[0]}'"
    exit 1
fi
#-- Extract the SCSI host number
SCSI_HOST="${SCSI_HOST_NAME:4}"
echo "[INFO] The corresponding SCSI host is 'host${SCSI_HOST}'"
...
if [[ ${#PHY_CHANGERS[@]} = 0 ]]; then
    echo "[ERROR] No physical medium changer device found!"
    exit 1
else
    for PHY_CHANGER in "${PHY_CHANGERS}"; do
        if [[ ! ${PHY_CHANGER} =~ "/dev/sch" ]]; then
            echo "[ERROR] Expecting physical medium changer device name to match '/dev/sch' but found '${PHY_CHANGER}'!"
            exit 1
        fi
    done
fi
...
if [[ ${#GEN_CHANGERS[@]} = 0 ]]; then
    echo "[ERROR] No generic medium changer device found!"
    exit 1
else
    for GEN_CHANGER in "${GEN_CHANGERS}"; do
        if [[ ! ${GEN_CHANGER} =~ "/dev/sg" ]]; then
            echo "[ERROR] Expecting generic medium changer device name to match '/dev/sg' but found '${GEN_CHANGER}'!"
            exit 1
        fi
    done
fi
...
if [ ${VERBOSE} -gt 0 ]; then
    if [[ ${#GEN_CHANGERS[@]} -gt 1 ]]; then
        echo "[INFO] Found ${#GEN_CHANGERS[@]} medium changers."
        echo "[INFO] Selecting the first one: '${GEN_CHANGER}'"
    else
        echo "[INFO] Found medium changer to be '${GEN_CHANGER}'"
    fi
fi
...
if [[ ${#PHY_DRIVES[@]} = 0 ]]; then
    echo "[ERROR] No physical tape drive device found!"
    exit 1
else
    for PHY_DRIVE in "${PHY_DRIVES}"; do
        if [[ ! ${PHY_DRIVE} =~ "/dev/st" ]]; then
            echo "[ERROR] Expecting physical tape drive name to match '/dev/st' but found '${PHY_DRIVE}'!"
            exit 1
        fi
    done     
fi
...
if [[ ${#GEN_DRIVES[@]} = 0 ]]; then
    echo "[ERROR] No generic tape drive device found!"
    exit 1
else
    for GEN_DRIVE in "${GEN_DRIVES}"; do
        if [[ ! ${GEN_DRIVE} =~ "/dev/sg" ]]; then
            echo "[ERROR] Expecting generic tape drive device name to match '/dev/sg' but found '${GEN_DRIVE}'!"
            exit 1
        fi
    done
fi
...
if [ ${VERBOSE} -gt 0 ]; then
    if [[ ${#GEN_DRIVES[@]} -gt 1 ]]; then
        echo "[INFO] Found ${#GEN_DRIVES[@]} tape drives."
        echo "[INFO] Selecting the first one: '${GEN_DRIVE}'"
    else
        echo "[INFO] Found tape drive to be '${GEN_DRIVE}'"
    fi
fi

echo "#-- Check and clear existing SCSI reservations -----------"
...
for GEN_DEV in "${GEN_DEVS[@]}"; do
    ...
    if [[ ${RES} =~ "Unit attention" ]]; then
        echo "[WARNING] The SCSI device '${GEN_DEV}'experienced a Unit Attention condition in the past. ${RES}"
        echo "[INFO] Trying to read the SCSI reservation a second time..."
        RES=$(sudo sg_persist --in --read-reservation ${GEN_DEV})
    fi
    if [ $? -ne 0 ]; then
        echo "[ERROR] Failed to read SCSI reservations for '${GEN_DEV}'"
        exit 1
    elif [ ${VERBOSE} -gt 0 ]; then
        echo "[INFO] Current SCSI reservation for '${GEN_DEV}' "
        echo "${RES}"
    fi
    if [[ ${RES} =~ "Reservation follows:" ]]; then
        echo "[WARNING] A SCSI reservations is in place for '${GEN_DEV}' "
        # Retrieve the reservation type
        RES_TYPE=$(echo "${RES}" | grep -oP ',  type: \K.*')
        if [ $? -ne 0 ]; then
            echo "[ERROR] Failed to retrieve the SCSI reservation type for '${GEN_DEV}' "
            exit 1
        elif [ ${VERBOSE} -gt 0 ]; then
            echo "[INFO] The reservation type for '${GEN_DEV}' is: '${RES_TYPE}'"
        fi
        # Retrieve the reservation key
        RES_KEY=$(echo "${RES}" | grep -oP 'Key=\K0x[0-9a-fA-F]+')
        if [ $? -ne 0 ]; then
            echo "[ERROR] Failed to retrieve the reservation key for '${GEN_DEV}' "
            exit 1
        elif [ ${VERBOSE} -gt 0 ]; then
            echo "[INFO] The reservation key for  '${GEN_DEV}' is: '${RES_KEY}'"
        fi
        # Clear the reservation
        if [[ "${RES_TYPE}" == "Exclusive Access" ]]; then
            RES=$(sudo sg_persist --out --clear --param-rk=${RES_KEY} ${GEN_DEV})
            if [ $? -ne 0 ]; then
                echo "[ERROR] Failed to clear the SCSI '${RES_TYPE}}' reservation for '${GEN_DEV}'"
                exit 1
            elif [ ${VERBOSE} -gt 0 ]; then
                echo "[INFO] Cleared the SCSI '${RES_TYPE}}' reservation for '${GEN_DEV}'"
            fi
        else
...
then
    ...
    echo -e "\t The medium changer device is: '${GEN_CHANGER}' "
    echo -e "\t The tape drive device is    : '${GEN_DRIVE}' "
    echo
