...
if [ ! -d "${TMFS_LOG_DIR}" ]; then
    echo -e "[❌]${RED} Directory '${TMFS_LOG_DIR}' does not exist. ${NC}"
    exit 1
elif [ ${VERBOSE} -gt 0 ]; then
    echo -e "[✅] TMFS_LOG_DIR directory is: '${TMFS_LOG_DIR}' "
fi
...
touch ${TMFS_INIT_LOG}
if [ $? -ne 0 ]; then
    echo -e "[❌]${RED} Failed to create the TMFS_INIT log file... ${NC}"
    exit 1
elif [ ${VERBOSE} -gt 0 ]; then
...
if [ ! -d "${TMFS_DATA_DIR}" ]; then
  echo -e "[❌]${RED} Directory '${TMFS_DATA_DIR}' does not exist... ${NC}" >> ${TMFS_INIT_LOG}
  exit 1
elif [ ${VERBOSE} -gt 0 ]; then
...
if [ ! -d "${TMFS_WORK_DIR}" ]; then
    echo -e "[❌]${RED} Directory '${TMFS_WORK_DIR}' does not exist... ${NC}" >> ${TMFS_INIT_LOG}
    exit 1
elif [ ${VERBOSE} -gt 0 ]; then
    echo -e "[✅] TMFS_WORK_DIR directory is: '${TMFS_WORK_DIR}' " >> ${TMFS_INIT_LOG}
fi
...
if [ -z "${BASEDEV}" ]; then
    echo -e "[❌]${RED} BASEDEV is is empty. ${NC}" >> ${TMFS_INIT_LOG}
    exit 1;
elif [ ${VERBOSE} -gt 0 ]; then
    echo -e "[✅] BASEDEV=${BASEDEV}" >> ${TMFS_INIT_LOG}
fi

echo "#-- Setting ADMIN_KEY ------------------------------------" >> ${TMFS_INIT_LOG}
if [ -z "${CEPH_ADMIN_KEY}" ]; then
    echo -e "[❌]${RED} Secret CEPH_ADMIN_KEY is not set... ${NC}" >> ${TMFS_INIT_LOG}
    exit 1
elif [ ${VERBOSE} -gt 0 ]; then
    echo -e "[✅] CEPH_ADMIN_KEY=${CEPH_ADMIN_KEY}" >> ${TMFS_INIT_LOG}
fi
...
if [[ ${#PCI_ADDRS[@]} = 0 ]]; then
    echo -e "[❌]${RED} No Fibre Channel host buffer adapter found! ${NC}" >> ${TMFS_INIT_LOG}
    exit 1
elif [ ${VERBOSE} -gt 0 ]; then
    echo -e "[INFO] Found ${#PCI_ADDRS[@]} Fibre Channel ports" >> ${TMFS_INIT_LOG}
fi
echo -e "[✅] Selecting Fibre Channel HBA located at PCI address '${PCI_ADDRS[0]}'" >> ${TMFS_INIT_LOG}
...
echo -e "[✅] The corresponding SCSI host is 'host${SCSI_HOST}'" >> ${TMFS_INIT_LOG}
...
if [[ ${#PHY_CHANGERS[@]} = 0 ]]; then
    echo -e "[❌]${RED} No physical medium changer device found! ${NC}" >> ${TMFS_INIT_LOG}
    exit 1
else
    for PHY_CHANGER in "${PHY_CHANGERS}"; do
        if [[ ! ${PHY_CHANGER} =~ "/dev/sch" ]]; then
            echo -e "[❌]${RED} Expecting physical medium changer device name to match '/dev/sch' but found '${PHY_CHANGER}'! ${NC}" >> ${TMFS_INIT_LOG}
            exit 1
        fi
    done
fi
...
if [[ ${#GEN_CHANGERS[@]} = 0 ]]; then
    echo -e "[❌]${RED} No generic medium changer device found! ${NC}" >> ${TMFS_INIT_LOG}
    exit 1
else
    for GEN_CHANGER in "${GEN_CHANGERS}"; do
        if [[ ! ${GEN_CHANGER} =~ "/dev/sg" ]]; then
            echo -e "[❌]${RED} Expecting generic medium changer device name to match '/dev/sg' but found '${GEN_CHANGER}'! ${NC}" >> ${TMFS_INIT_LOG}
            exit 1
        fi
    done
fi
...
if [ ${VERBOSE} -gt 0 ]; then
    if [[ ${#GEN_CHANGERS[@]} -gt 1 ]]; then
        echo -e "[INFO] Found ${#GEN_CHANGERS[@]} medium changers." >> ${TMFS_INIT_LOG}
        echo -e "[✅] Selecting the first one: '${GEN_CHANGER}'" >> ${TMFS_INIT_LOG}
    else
        echo -e "[✅] Found medium changer to be '${GEN_CHANGER}'" >> ${TMFS_INIT_LOG}
    fi
fi
...
if [[ ${#PHY_DRIVES[@]} = 0 ]]; then
    echo -e "[❌]${RED} No physical tape drive device found! ${NC}" >> ${TMFS_INIT_LOG}
    exit 1
else
    for PHY_DRIVE in "${PHY_DRIVES}"; do
        if [[ ! ${PHY_DRIVE} =~ "/dev/st" ]]; then
            echo -e "[❌]${RED} Expecting physical tape drive name to match '/dev/st' but found '${PHY_DRIVE}'! ${NC}" >> ${TMFS_INIT_LOG}
            exit 1
        fi
    done     
fi
...
if [[ ${#GEN_DRIVES[@]} = 0 ]]; then
    echo -e "[❌]${RED} No generic tape drive device found! ${NC}" >> ${TMFS_INIT_LOG}
    exit 1
else
    for GEN_DRIVE in "${GEN_DRIVES}"; do
        if [[ ! ${GEN_DRIVE} =~ "/dev/sg" ]]; then
            echo -e "[❌]${RED} Expecting generic tape drive device name to match '/dev/sg' but found '${GEN_DRIVE}'! ${NC}" >> ${TMFS_INIT_LOG}
            exit 1
        fi
    done
fi
...
if [ ${VERBOSE} -gt 0 ]; then
    if [[ ${#GEN_DRIVES[@]} -gt 1 ]]; then
        ...
        echo -e "[✅] Selecting the first one: '${GEN_DRIVE}'" >> ${TMFS_INIT_LOG}
    else
...
for GEN_DEV in "${GEN_DEVS[@]}"; do
    ...
    if [[ ${RES} =~ "Reservation follows:" ]]; then
        ...
        if [ $? -ne 0 ]; then
            echo -e "[❌]${RED} Failed to retrieve the SCSI reservation type for '${GEN_DEV}' ${NC}" >> ${TMFS_INIT_LOG}
            exit 1
        elif [ ${VERBOSE} -gt 0 ]; then
            echo -e "[INFO] The reservation type for '${GEN_DEV}' is: '${RES_TYPE}'" >> ${TMFS_INIT_LOG}
        fi
        # Retrieve the reservation key
        RES_KEY=$(echo "${RES}" | grep -oP 'Key=\K0x[0-9a-fA-F]+')
        if [ $? -ne 0 ]; then
            echo -e "[❌]${RED} Failed to retrieve the reservation key for '${GEN_DEV}' ${NC}" >> ${TMFS_INIT_LOG}
            exit 1
        elif [ ${VERBOSE} -gt 0 ]; then
            echo -e "[INFO] The reservation key for  '${GEN_DEV}' is: '${RES_KEY}'" >> ${TMFS_INIT_LOG}
        fi
        # Clear the reservation
        if [[ "${RES_TYPE}" == "Exclusive Access" ]]; then
            RES=$(sudo sg_persist --out --clear --param-rk=${RES_KEY} ${GEN_DEV})
            if [ $? -ne 0 ]; then
                echo -e "[❌]${RED} Failed to clear the SCSI '${RES_TYPE}}' reservation for '${GEN_DEV}' ${NC}" >> ${TMFS_INIT_LOG}
                exit 1
            elif [ ${VERBOSE} -gt 0 ]; then
                echo -e "[✅] Cleared the SCSI '${RES_TYPE}}' reservation for '${GEN_DEV}'" >> ${TMFS_INIT_LOG}
            fi
        else
            echo -e "[❌]${RED} The clearing of the SCSI reservation type '${RES_TYPE}' is not supported yet... ${NC}" >> ${TMFS_INIT_LOG}
            exit 1
        fi
        ...
        if [ $? -ne 0 ]; then
            ...
            echo -e "[❌]${RED} One or more SCSI reservation(s) still pending... ${NC}" >> ${TMFS_INIT_LOG}
            exit 1
        fi
    fi
done
...
then
    echo -e "[✅] TMFS is already running." >> ${TMFS_INIT_LOG}
else   
    # Recap all the settings
    echo -e "[✅] Summary of the TMFS settings:" >> ${TMFS_INIT_LOG}
    echo -e "\t TMFS data directory is      : '${TMFS_DATA_DIR}' " >> ${TMFS_INIT_LOG}
    ...
    if true; then
        ...
        if mountpoint -q ${TMFS_DATA_DIR}; then
            if ! sudo umount ${TMFS_DATA_DIR}; then
                echo -e "[❌]${RED} Failed to unmount '${TMFS_DATA_DIR}'... ${NC}" >> ${TMFS_INIT_LOG}
                exit 1
            fi
        else
            echo -e "[✅] '${TMFS_DATA_DIR}' was not mounted" >> ${TMFS_INIT_LOG}
        fi
